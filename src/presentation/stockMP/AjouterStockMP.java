                             ...
                             BigDecimal montantTotalEmballage=BigDecimal.ZERO;
		  					...
		  					for(int i=0;i<listDetailTransfertMp.size();i++)
		  					{
		  						...
		  						if(detailTransferStockMP.getMatierePremier().getCategorieMp().getSubCategorieMp().getCode().equals(SOUS_CATEGORIE_MATIERE_PREMIERE_THE))
		  		     			{
		  		     				montantTotalEnvrac=montantTotalEnvrac.add(detailTransferStockMP.getQuantite().multiply(detailTransferStockMP.getMatierePremier().getPrixInitial2024()));
		  		     				 
		  		     				
		  		     			}else
		  		     			{
		  		     				montantTotalEmballage=montantTotalEmballage.add(detailTransferStockMP.getQuantite().multiply(detailTransferStockMP.getMatierePremier().getPrixInitial2024()));
		  		     				 
		  		     			}
		  						
		  		     			montantTotal=montantTotal.add(detailTransferStockMP.getQuantite().multiply(detailTransferStockMP.getMatierePremier().getPrixInitial2024()));
