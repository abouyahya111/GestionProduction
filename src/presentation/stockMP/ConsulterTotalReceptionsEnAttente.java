...
public class ConsulterTotalReceptionsEnAttente extends JLayeredPane implements Constantes{
	...
	private List<ConsulterReceptionTotal> listReceptionTotal=new ArrayList<ConsulterReceptionTotal>();
	 ...
	 */
	public ConsulterTotalReceptionsEnAttente() {
		setOpaque(true);
       ...
       comboMagasinDestination=new JComboBox();
        ...
        String requete="";
				...
				if(!txtReftransfer.getText().equals(""))
				{
					requete= requete+" and transferStockMP.CodeTransfer='"+txtReftransfer.getText()+"'";
				}
				
				requete=requete+" and transferStockMP.statut='"+Constantes.ETAT_RECEPTION_ENATTENT+"'";
				...
				for(int t=0;t<listDetailTransfertMP.size();t++)
				{
					existe=false;
				DetailTransferStockMP detailTransferStockMP=listDetailTransfertMP.get(t);
				
				for(int j=0;j<listReceptionTotal.size();j++)
				{
					
					ConsulterReceptionTotal consulterReceptionTotal=listReceptionTotal.get(j);
					
					if(detailTransferStockMP.getMatierePremier().getId()==consulterReceptionTotal.getMatierePremier().getId())
					{
						if(detailTransferStockMP.getMatierePremier().getCategorieMp().getSubCategorieMp().getCode().equals(Constantes.SOUS_CATEGORIE_MATIERE_PREMIERE_THE))
						{
							if(detailTransferStockMP.getFournisseur()!=null)
							{
								...
								if(detailTransferStockMP.getFournisseur().getId()==consulterReceptionTotal.getFournisseur().getId())
								{
									consulterReceptionTotal.setQuantiteTotal (consulterReceptionTotal.getQuantiteTotal().add(detailTransferStockMP.getQuantite()));
									existe=true;
								}
					...
					if(existe==false)
					{
						if(detailTransferStockMP.getMatierePremier().getCategorieMp().getSubCategorieMp().getCode().equals(Constantes.SOUS_CATEGORIE_MATIERE_PREMIERE_THE))
						{
							ConsulterReceptionTotal consulterReceptionTotalTmp=new ConsulterReceptionTotal();	
							consulterReceptionTotalTmp.setMatierePremier(detailTransferStockMP.getMatierePremier());
							consulterReceptionTotalTmp.setQuantiteTotal(detailTransferStockMP.getQuantite());
							consulterReceptionTotalTmp.setFournisseur(detailTransferStockMP.getFournisseur());
							listReceptionTotal.add(consulterReceptionTotalTmp);
						...
						}else
						{
							...
							listReceptionTotal.add(consulterReceptionTotalTmp);
				  					...
				  					afficher_tableMP(listReceptionTotal);
				  				/*	}else {
		...
		btnBonEntrerMp.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				 ...
				 if(listReceptionTotal.size()!=0)
				 {
						...
						JasperUtils.HistoriqueReceptionTotalStockMP (listReceptionTotal,parameters);
	  ...
	  if(!utilisateur.getCodeDepot().equals(CODE_DEPOT_SIEGE)	) {
		...
		lblService.setBounds(542, 64, 71, 24);
		layeredPane.add(lblService);
		
		 comboService = new JComboBox();
		comboService.setBounds(623, 64, 255, 24);
...
txtCodeMP.setText(Constantes.CODE_MP);
	...
	void afficher_tableMP(List<ConsulterReceptionTotal> listConsulterReceptionTotal)
	{

		
			intialiserTableau();
			 int i=0;
				while(i<listConsulterReceptionTotal.size())
				{	
					
					ConsulterReceptionTotal ConsulterReceptionTotal=listConsulterReceptionTotal.get(i);
					String fournisseur="";
					if(ConsulterReceptionTotal.getFournisseur()!=null)
					{
						fournisseur=ConsulterReceptionTotal.getFournisseur().getCodeFournisseur();
					}

						Object []ligne={ConsulterReceptionTotal.getMatierePremier().getCode(),ConsulterReceptionTotal.getMatierePremier().getNom(), fournisseur,  NumberFormat.getNumberInstance(Locale.FRANCE).format(ConsulterReceptionTotal.getQuantiteTotal())};
...
void intialiserTableau(){
	...
	modeleMP =new DefaultTableModel(
		     	...
		     	new String[] {
		     		"Code MP","Nom MP","Fournisseur","Quantité"
		     	}
		     ) {
