	...
	intialiserTableau();
		  ...
		  BigDecimal prixInitial=BigDecimal.ZERO;
			...
			{	
				...
				MatierePremier mp=matierePremiereDAO.findByCode(situationEnVrac.getCodematierepremiere());
				if(mp.getPrix2024()!=null)
				{
					prix=mp.getPrix2024();
				} 
				if(mp.getPrixInitial2024()!=null)
				{
					prixInitial=mp.getPrixInitial2024();
				} 
			
				Object []ligne={situationEnVrac.getCodematierepremiere(),situationEnVrac.getMatierepremiere(),dfDecimal.format(situationEnVrac.getEnattent().setScale(4, RoundingMode.HALF_UP)),dfDecimal.format(situationEnVrac.getTantan().setScale(4, RoundingMode.HALF_UP)),dfDecimal.format(situationEnVrac.getLaayoun().setScale(4, RoundingMode.HALF_UP)),dfDecimal.format(situationEnVrac.getEssmara().setScale(4, RoundingMode.HALF_UP)),dfDecimal.format(situationEnVrac.getAgadir().setScale(4, RoundingMode.HALF_UP)),dfDecimal.format(situationEnVrac.getTotal().setScale(4, RoundingMode.HALF_UP)),prix,prixInitial  };
...
void intialiserTableau(){
	 ...
	 modeleMP =new DefaultTableModel(
		     	...
		     	new String[] {
		     			"Code MP","Matière Première",  "TRF EN ATTENTE", "TANTAN", "LAAYOUN", "ESSMARA", "AGADIR", "TOTAL","Prix","Prix Initial"
		     	}
		     ) {
