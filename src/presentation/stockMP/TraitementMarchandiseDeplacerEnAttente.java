				 ...
				 int reponse = JOptionPane.showConfirmDialog(null, "Vous voulez vraiment Valider l'opération  ?", 
						...
						
					{
						...
						for(int c=0;c<listDetailMarchandiseDeplacer.size();c++)		
						{
							...
							if(listDetailMarchandiseDeplacer.get(c).getMatierePremier().getPrixByYear(DateUtils.getAnnee(listDetailMarchandiseDeplacer.get(c).getDetailTransferStockMP().getTransferStockMP().getDateTransfer())).compareTo(BigDecimal.ZERO)==0)
							{
								ErreurPrix=true;
					...
					for(int d=0;d<listDetailMarchandiseDeplacer.size();d++)		
					{
						...
						}else
						{
							...
							if(detailTransferStockMP.getAction()!=null)
							{
								...
								{
									if(DateUtils.getAnnee(detailTransferStockMP.getTransferStockMP().getDateTransfer())==2023)
									{
									...
									{
										listDetailMarchandiseDeplacer.get(t).setMontant(listDetailMarchandiseDeplacer.get(t).getMatierePremier().getPrixByYear(DateUtils.getAnnee(detailTransferStockMP.getTransferStockMP().getDateTransfer())).multiply(listDetailMarchandiseDeplacer.get(t).getQuantiteManque()).setScale(2, RoundingMode.HALF_UP));
