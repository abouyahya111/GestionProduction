...
public class ConsulterInitialMP extends JLayeredPane implements Constantes {
	...
	private JTextField txtcodeMPModifier;
	private JTextField txtPrixInitial;
	JComboBox comboMpModifier = new JComboBox();
	...
	public ConsulterInitialMP() {
		...
		tableEtatStock.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				EtatStockMP etatStockMP=listEtatStockMP.get(tableEtatStock.getSelectedRow());
				
					txtcodeMPModifier.setText(etatStockMP.getMp().getCode());
				comboMpModifier.setSelectedItem(etatStockMP.getMp().getNom());
				txtPrixInitial.setText (new BigDecimal( NumberFormat.getNumberInstance(Locale.FRANCE).format(
						etatStockMP.getPrixInitial().setScale(6, RoundingMode.HALF_UP)).replace(",", "."))  +"");
				ligne=tableEtatStock.getSelectedRow();
		...
		scrollPane.setBounds(10, 344, 1465, 587);
		add(scrollPane);
		scrollPane.setBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null));

		JXTitledSeparator titledSeparator = new JXTitledSeparator();
		titledSeparator.setTitle("Etat de Stock");
		titledSeparator.setBounds(10, 303, 1465, 30);
		add(titledSeparator);
		...
		buttonvalider.addActionListener(new ActionListener() {
			...
			if(magasin!=null)
			{
				 if(listEtatStockMP.size()!=0)
				 {
						...
						JasperUtils.imprimerEtatInitialStockMP(listEtatStockMP,parameters);
		...
		buttonvalider.setBounds(698, 942, 112, 32);
...
return;
				...
				if(DateUtils.getAnnee(dateChooserdebut.getDate())==2023)
				{
					...
					for(int i=0;i<listeObjectStockInitial.size() ; i++)
					{
						...
						etatStockMP.setAnne(DateUtils.getAnnee(dateChooserdebut.getDate()));
		...
		comboMP.setBounds(557, 57, 306, 24);
		...
		button.setIcon(imgInit);
 		...
 		label.setBounds(10, 221, 67, 24);
 		add(label);
 		
 		txtcodeMPModifier = new JTextField();
 		txtcodeMPModifier.addKeyListener(new KeyAdapter() {
 			@Override
 			public void keyPressed(KeyEvent e) {
				...
				if(e.getKeyCode()==e.VK_ENTER)
	      		{
					MatierePremier mp=MapCodeMatierPremiere.get(txtcodeMPModifier.getText().toUpperCase());
					if(mp!=null)
					{
						comboMpModifier.setSelectedItem(mp.getNom());
					}else
					{
						JOptionPane.showMessageDialog(null, "Code MP Introuvable !!!!!");
						return;
 		...
 		txtcodeMPModifier.setText("");
 		txtcodeMPModifier.setColumns(10);
 		txtcodeMPModifier.setBounds(70, 221, 118, 26);
 		add(txtcodeMPModifier);
 		
 		JLabel label_6 = new JLabel("MP :");
 		label_6.setFont(new Font("Tahoma", Font.PLAIN, 11));
 		label_6.setBounds(198, 223, 37, 24);
 		add(label_6);
 		
 		  comboMpModifier = new JComboBox();
 		  comboMpModifier.addItemListener(new ItemListener() {
 		  	public void itemStateChanged(ItemEvent e) {


	  		 	
  		 		if(comboMpModifier.getSelectedIndex()!=-1)
  		 		{
  		 			
  		 			if(!comboMpModifier.getSelectedItem().equals(""))
  		 			{
  		 				
  		 				MatierePremier matierePremier=MapMatierPremiere.get(comboMpModifier.getSelectedItem());
  		 				if(matierePremier!=null)
  		 				{
  		 					txtcodeMPModifier.setText(matierePremier.getCode());
  		 				}
  		 			...
  		 			{
  		 				txtcodeMPModifier.setText(Constantes.CODE_MP);
  		 			}
 		...
 		comboMpModifier.setSelectedIndex(-1);
 		comboMpModifier.setBounds(232, 223, 291, 24);
 		add(comboMpModifier);
 		
 		JLabel lblPrixInitial = new JLabel("Prix Initial :");
 		lblPrixInitial.setFont(new Font("Tahoma", Font.PLAIN, 13));
 		lblPrixInitial.setBounds(546, 221, 79, 24);
 		add(lblPrixInitial);
 		
 		txtPrixInitial = new JTextField();
 		txtPrixInitial.setText("");
 		txtPrixInitial.setColumns(10);
 		txtPrixInitial.setBounds(632, 221, 126, 26);
 		add(txtPrixInitial);
 		...
 		btnModifier.addActionListener(new ActionListener() {
 			public void actionPerformed(ActionEvent arg0) {
 				...
 				if(!txtPrixInitial.getText().equals(""))
 				{
 					
 					if(new BigDecimal(txtPrixInitial.getText()).compareTo(BigDecimal.ZERO)==0)
 					{
 						JOptionPane.showMessageDialog(null, "le prix doit etre supérieur à 0 SVP");
 	 					return;
 						
 					}else
 					{
 						
 						EtatStockMP etatStockMP=listEtatStockMP.get(ligne);
 						
 						etatStockMP.setPrixInitial(new BigDecimal(txtPrixInitial.getText()));
 						listEtatStockMP.set(ligne, etatStockMP);
 						MatierePremier matierePremier=etatStockMP.getMp();
 						
 						matierePremier.setPrixInitialByYear(etatStockMP.getAnne(), new BigDecimal(txtPrixInitial.getText()));
 						matierePremiereDAO.edit(matierePremier);
 						afficher_tableEtatStock(listEtatStockMP);
 						
 						txtcodeMPModifier.setText("");
 						comboMpModifier.setSelectedItem("");
 						txtPrixInitial.setText("");
 		...
 		btnModifier.setBounds(242, 268, 107, 24);
 		add(btnModifier);
		...
		comboMpModifier.addItem("");
		comboMP.addItem("");
		for(int t=0;t<listMP.size();t++)
		{
			MatierePremier matierePremier=listMP.get(t);
			comboMP.addItem(matierePremier.getNom());
			comboMpModifier.addItem(matierePremier.getNom());
			MapMatierPremiere.put(matierePremier.getNom(), matierePremier);
			MapCodeMatierPremiere.put(matierePremier.getCode(), matierePremier);
		}
...
btnVider.setFont(new Font("Tahoma", Font.PLAIN, 11));
btnVider.setBounds(376, 268, 107, 24);
add(btnVider);
btnVider.setIcon(imgInit);
	}
	...
	void afficher_tableEtatStock(List<EtatStockMP> listEtatStockMP) {
		modeleEtatStock = new DefaultTableModel(new Object[][] {},
				new String[] { "Code","Matiere Premiere", "Prix" }) {
