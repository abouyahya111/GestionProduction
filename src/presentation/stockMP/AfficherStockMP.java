  					...
  					if(utilisateur.getType()!=null)
						{
							...
							{
								subCategorieMp=subCategorieMpthe;
							}
	  			...
	  			if(utilisateur.getType()!=null)
					{
						if(utilisateur.getType().equals(Constantes.UTILISATEUR_SORTIE_ENVRAC))
						{
							if(listsubcategoriemp.get(i).getCode().equals(Constantes.SOUS_CATEGORIE_MATIERE_PREMIERE_THE))
								{
	 ...
	 BigDecimal prixInitial=BigDecimal.ZERO;
		  int i=0;
			...
			{	
				...
				if(EtatstockMP.getMp().getType().equals(Constantes.MP_INTERNE))
				{
					
					if(EtatstockMP.getMp().getPrixByYear( DateUtils.getAnnee(dateSituation.getDate()))==null)
					{
						QuantiteEnAttente=EtatstockMP.getQuantiteFinale();
						
					}else
					{
						if(EtatstockMP.getMp().getPrixByYear( DateUtils.getAnnee(dateSituation.getDate())).compareTo(BigDecimal.ZERO)==0)
  		  				{
							QuantiteEnAttente=EtatstockMP.getQuantiteFinale();
  		  				}
					}
				...
				if(EtatstockMP.getMp().getPrixInitial2024()!=null)
				{
					prixInitial=EtatstockMP.getMp().getPrixInitial2024();
				} 
			...
			//	mapPrixMP.put(stockMP.getMatierePremier().getCode(), stockMP.getPrixUnitaire());
				...
				EtatstockMP.setQuantiteEnAttente(QuantiteEnAttente);
				
				if(EtatstockMP.getMp().getType().equals(Constantes.MP_INTERNE))
				{
					if(QuantiteEnAttente.compareTo(BigDecimal.ZERO)!=0)
					{
						
						EtatstockMP.setQuantiteFinale(BigDecimal.ZERO);
						EtatstockMP.setQuantiteTotal(EtatstockMP.getQuantiteEnAttente());
						Object []ligne={EtatstockMP.getMp().getCode(),EtatstockMP.getMp().getNom(),dfDecimal.format(BigDecimal.ZERO),EtatstockMP.getQuantiteEnAttente(),dfDecimal.format(EtatstockMP.getQuantiteEnAttente()),fournisseurMP,EtatstockMP.getMp().getPrix2024(),EtatstockMP.getQuantiteEnAttente().multiply(EtatstockMP.getMp().getPrix2024()),prixInitial};
					...
					}else
					{
						...
						EtatstockMP.setQuantiteTotal(EtatstockMP.getQuantiteEnAttente().add(EtatstockMP.getQuantiteFinale()));
						
						Object []ligne={EtatstockMP.getMp().getCode(),EtatstockMP.getMp().getNom(),dfDecimal.format(EtatstockMP.getQuantiteFinale()),dfDecimal.format(EtatstockMP.getQuantiteEnAttente()),dfDecimal.format(EtatstockMP.getQuantiteFinale().add(EtatstockMP.getQuantiteEnAttente())),fournisseurMP,EtatstockMP.getMp().getPrix2024(),(EtatstockMP.getQuantiteFinale().add(EtatstockMP.getQuantiteEnAttente())).multiply(EtatstockMP.getMp().getPrix2024()) ,prixInitial};
				...
				{
					EtatstockMP.setQuantiteTotal(EtatstockMP.getQuantiteFinale().add(EtatstockMP.getQuantiteEnAttente()));
					
					Object []ligne={EtatstockMP.getMp().getCode(),EtatstockMP.getMp().getNom(),dfDecimal.format(EtatstockMP.getQuantiteFinale()),dfDecimal.format(BigDecimal.ZERO),dfDecimal.format(EtatstockMP.getQuantiteFinale()),fournisseurMP,EtatstockMP.getMp().getPrix2024(),EtatstockMP.getQuantiteFinale().multiply(EtatstockMP.getMp().getPrix2024()),prixInitial};
...
void intialiserTableau(){
	 ...
	 modeleMP =new DefaultTableModel(
		     	...
		     	new String[] {
		     			...
		     			"Code MatiËre PremiËre","MatiËre PremiËre", "QuantitÈ","QuantitÈ En Attente","Total","Fournisseur","prix","Montant","Prix Initial"
		     	}
		     ) {
