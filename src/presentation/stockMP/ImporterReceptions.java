...
import dao.entity.importerReception;
...
public class ImporterReceptions extends JLayeredPane implements Constantes{
	...
	List<importerReception> listImportationReception =new ArrayList<importerReception>();
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////					
		...
		layeredPane_1.add(txtcode);
		  ...
		  labelCodeReception = new JLabel("Code R\u00E9ception :");
		...
		layeredPane_1.add(label_3);
		 ...
		 btnImporterReceptionExcel.addActionListener(new ActionListener() {
		 	public void actionPerformed(ActionEvent arg0) {
  			...
  			Date datecommande=dateDebut.getDate();	
  			
  			if(datecommande==null)
  			{
  				JOptionPane.showMessageDialog(null, "Veuillez Selectionner date debut SVP");
	        	return;
  			}
...
if(matierePremier.getPrixByYear(DateUtils.getAnnee(datecommande) )!=null)
{
	detailTransferStockMP.setPrixUnitaire(matierePremier.getPrixByYear(DateUtils.getAnnee(datecommande) ));
}else
{
...
if(matierePremier.getPrixByYear(DateUtils.getAnnee(datecommande) )!=null)
{
	detailTransferStockMPTmp.setPrixUnitaire(matierePremier.getPrixByYear(DateUtils.getAnnee(datecommande) ));
}else
{
	...
	try
		{
		 ...
		 String	 query="select db.DATE_DETAIL_BON_LIVR, db.NUM_LIVRE_FOUR, db.NUM_RECEPTION, mp.code,mp.nom,db.Quantite  from detail_bon_livraison db , matiere_premier mp, categorie_mp c , sub_categorie_mp s where db.id_mat_pre=mp.id and mp.id_cat=c.ID and c.id_sub_cat=s.id and s.id!=16 and db.NUM_RECEPTION is not null and db.NUM_LIVRE_FOUR='"+numBLCommande+"' " ; 
		 String msg="";		 
...
List<String> ListeNumReception=new ArrayList<>();
	...
	}else if(magasin==null)
	{
		...
		listImportationReception.clear();
...
if(!daterecption.equals(mapDateNumReception.get(table.getValueAt(d, 2).toString())))
{
	DateReceptionNonDeclarerOU=true;
			 ...
			 if(DateReceptionNonDeclarerOU==true)
			 {
				...
				for(int i=0;i<listDetailTransferStockMP.size();i++)
				{
					...
					{
						Date daterecption=null;
						
						try {
							 daterecption= simpleDateFormat.parse(table.getValueAt(i, 7).toString());
							
						
							
						} catch (Exception e2) {
							JOptionPane.showMessageDialog(null, "Erreur de date Dans la ligne  "+(i+1) +" Veuillez Entrer La Date En Format Suivante SVP : Annee-Mois-Jours");
							return;
						}	
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////    Ajout   ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////				
						...
						if(mp.getPrixByYear(DateUtils.getAnnee(daterecption))!=null)
						{
							detailTransferStockAjout.setPrixUnitaire(mp.getPrixByYear(DateUtils.getAnnee(daterecption)));
						}else
						{
						...
						if(detailTransferStockAjout.getMatierePremier().getCategorieMp().getSubCategorieMp().getCode().equals(SOUS_CATEGORIE_MATIERE_PREMIERE_THE))
  		     			{
  		     				montantTotalEnvrac= detailTransferStockAjout.getQuantite().multiply(detailTransferStockAjout.getMatierePremier().getPrixByYear(DateUtils.getAnnee(daterecption)));
  		     				 
  		     				
  		     			}else
  		     			{
  		     				montantTotalEmballage= detailTransferStockAjout.getQuantite().multiply(detailTransferStockAjout.getMatierePremier().getPrixByYear(DateUtils.getAnnee(daterecption)));
  		     				 
  		     			}
  		     			montantTotal= detailTransferStockAjout.getQuantite().multiply(detailTransferStockAjout.getMatierePremier().getPrixByYear(DateUtils.getAnnee(daterecption)));
  		     			 boolean trouver=false;
  						for(int d=0;d<listImportationReception.size();d++)
  								{
  									importerReception importerReception=listImportationReception.get(d);
  									
  									if(importerReception.getCode_transfert().equals(table.getValueAt(i, 2).toString()))
  									{
  										trouver=true;
  										importerReception.setMontantEmballage(importerReception.getMontantEmballage().add(montantTotalEmballage));
  										importerReception.setMontantEnVrac(importerReception.getMontantEnVrac().add(montantTotalEnvrac));
  										importerReception.setMontantTotal(importerReception.getMontantTotal().add(montantTotal));
  										listImportationReception.set(d, importerReception);
  									}
  								}
						...
						if(trouver==false)
						{
							...
							importerReception importerReception=new importerReception();	
							importerReception.setCode_transfert(table.getValueAt(i, 2).toString());
							importerReception.setMontantEmballage(montantTotalEmballage);
							importerReception.setMontantEnVrac(montantTotalEnvrac);
							importerReception.setMontantTotal(montantTotal);
							listImportationReception.add(importerReception);
						}
						...
						}else
						{
	                        ...
	                        detailTransferStockAjout.setMatierePremier(mp);
	                        detailTransferStockAjout.setQuantite(new BigDecimal(table.getValueAt(i,6).toString()));
						if(mp.getPrixByYear(DateUtils.getAnnee(transferStock.getDateTransfer()))!=null)
						{
							detailTransferStockAjout.setPrixUnitaire(mp.getPrixByYear(DateUtils.getAnnee(transferStock.getDateTransfer())));
						}else
						{
							detailTransferStockAjout.setPrixUnitaire(BigDecimal.ZERO);
						}
						...
						if(detailTransferStockAjout.getMatierePremier().getCategorieMp().getSubCategorieMp().getCode().equals(SOUS_CATEGORIE_MATIERE_PREMIERE_THE))
  		     			{
  		     				montantTotalEnvrac=detailTransferStockAjout.getQuantite().multiply(detailTransferStockAjout.getMatierePremier().getPrixByYear(DateUtils.getAnnee(transferStock.getDateTransfer())));
  		     				 
  		     				
  		     			}else
  		     			{
  		     				montantTotalEmballage=detailTransferStockAjout.getQuantite().multiply(detailTransferStockAjout.getMatierePremier().getPrixByYear(DateUtils.getAnnee(transferStock.getDateTransfer())));
  		     				 
  		     			}
  		     			montantTotal=detailTransferStockAjout.getQuantite().multiply(detailTransferStockAjout.getMatierePremier().getPrixByYear(DateUtils.getAnnee(transferStock.getDateTransfer())));
...
if(transfertexiste==false)
{
	listTransfertValider.add(listDetailTransferStockMPValider.get(j).getTransferStockMP());
				...
				for(int s=0;s<listImportationReception.size();s++)
				{
					...
					listeStatistiqueFinanciereMP=statistiqueFinanciereMPDAO.findAll();
		     		...
		     		if(statistiqueFinanciaireMPTmp!=null)
		     		{
		     			...
		     			statistiqueFinanciaireMP.setStockEmballage(statistiqueFinanciaireMPTmp.getStockEmballage().add(listImportationReception.get(s).getMontantEmballage()));
		     			statistiqueFinanciaireMP.setStockEnVrac(statistiqueFinanciaireMPTmp.getStockEnVrac().add(listImportationReception.get(s).getMontantEnVrac()));
		     			statistiqueFinanciaireMP.setCoutProduction(statistiqueFinanciaireMPTmp.getCoutProduction());
		     			statistiqueFinanciaireMP.setCodeTransfer(listImportationReception.get(s).getCode_transfert());
		     			...
		     			statistiqueFinanciaireMP.setCoutReception(listImportationReception.get(s).getMontantTotal().add(statistiqueFinanciaireMPTmp.getCoutReception()) );
		     			...
		     			statistiqueFinanciaireMP.setEtatTransfer(Constantes.ETAT_TRANSFER_STOCK_AJOUT);
...
void ValiderReceptionImporterViaGestionCommandeFinaleServeur()
{
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////    Reception Valider   ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////				
						...
						if(mp.getPrixByYear(DateUtils.getAnnee(daterecption))!=null)
						{
							detailTransferStockMPTmp.setPrixUnitaire(mp.getPrixByYear(DateUtils.getAnnee(daterecption)));
						}else
						{
							detailTransferStockMPTmp.setPrixUnitaire(BigDecimal.ZERO);
						}
						...
						if(mp.getPrixByYear(DateUtils.getAnnee(transferStock.getDateTransfer()))!=null)
						{
							detailTransferStockMPTmp.setPrixUnitaire(mp.getPrixByYear(DateUtils.getAnnee(transferStock.getDateTransfer())));
						}else
						{
							detailTransferStockMPTmp.setPrixUnitaire(BigDecimal.ZERO);
						}
