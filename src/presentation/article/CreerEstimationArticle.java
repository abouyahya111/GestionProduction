...
public class CreerEstimationArticle extends JLayeredPane {
	...
	private JTextField txtquantite =new JTextField();
	 ...
	 ClientDAO clientDAO;
	    ...
	    private static XSSFCell CellCodeCommerciale;
	...
	public CreerEstimationArticle() {
	  ...
	  AutoCompleteDecorator.decorate(comboClient);
		...
		txtlien.setBounds(504, 756, 384, 30);
		add(txtlien);
		...
		btnOuvrir.setFont(new Font("Tahoma", Font.PLAIN, 11));
		btnOuvrir.setBounds(911, 760, 112, 23);
		add(btnOuvrir);
		
		JButton btnChangerCodeArticle = new JButton("Changer Code Article");
		btnChangerCodeArticle.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
...
try {
	...
	Celllibelle=ExcelWSheet.getRow(i).getCell(0);
	CellCodeCommerciale=ExcelWSheet.getRow(i).getCell(1);
	CellCodeArticle=ExcelWSheet.getRow(i).getCell(2);
	 
	DataFormatter dataFormatter = new DataFormatter();
	String formattedlibelle = dataFormatter.formatCellValue(Celllibelle);
	String formattedCodeArticle = dataFormatter.formatCellValue(CellCodeArticle);
	
 if(formattedlibelle!=null && formattedlibelle.isEmpty()==false && !formattedlibelle.equals(""))
 {

 if(formattedCodeArticle!=null && formattedCodeArticle.isEmpty()==false && !formattedCodeArticle.equals(""))
 {
	 
	Articles articles=articlesDAO.findByCode(formattedCodeArticle) ;
	
	 if(articles!=null)
	 {
		 articles.setCodeArticle(CellCodeCommerciale.getStringCellValue());
		 articles.setLiblle(formattedlibelle);
		 articlesDAO.edit(articles);
	 }
 }else
 {
	...
	articles.setCodeArticle(CellCodeCommerciale.getStringCellValue());
	 articles.setLiblle(formattedlibelle);
	 articles.setPrixDetail(BigDecimal.ZERO);
	 articles.setPrixGros(BigDecimal.ZERO);
	 articles.setDateCreation(new Date());
	 articles.setClient(false);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////						  					
		...
		btnChangerCodeArticle.setFont(new Font("Tahoma", Font.PLAIN, 11));
		btnChangerCodeArticle.setBounds(345, 760, 140, 23);
		add(btnChangerCodeArticle);
		comboClient.setVisible(true);
