...
private List<Production> listProductionTerminer=new ArrayList<Production>();
				 ...
				 if(listFicheEmploye.size()!=0)
				 {
					 Parametre parametreNbrHeuresTotal=parametreDAO.findByLibelle(Constantes.NBR_HEURES_TOTAL);
				      String sommetowords = "";
	                    ...
	                    final FicheEmploye ficheEmploye = listFicheEmploye.get(0);
	                    Collections.sort(listFicheEmploye, new Comparateur());
	                    if(listFicheEmploye.get(0).getEmploye().getDatePremiereProduction()!=null)
	                    {
	...
	for(int j=0;j<listFicheEmploye.size();j++)
	{
		...
		ficheEmployeTmp.setCoutHoraire((ficheEmployeTmp.getCoutHoraire().multiply(primeAnciennte.getTaux().divide(new BigDecimal(100), 6, RoundingMode.HALF_UP))).add(ficheEmployeTmp.getCoutHoraire()));
	                    ...
	                    parameters.put("matricule", ficheEmploye.getEmploye().getMatricule());
	                    parameters.put("nom", ficheEmploye.getEmploye().getNomafficher());
	                    parameters.put("totalCout",totalcout.setScale(2, RoundingMode.HALF_UP).toString());
	                    parameters.put("totalAvance", totalheurtravauiller.setScale(2, RoundingMode.HALF_UP).toString());
	                    parameters.put("totalPrime", txtTotalPrime.getText());
	                    parameters.put("totalDu", totaldu.setScale(2, RoundingMode.HALF_UP).toString());
	                    parameters.put("COUT_HORAIRE", coutHoraire.setScale(2, RoundingMode.HALF_UP));
	                    parameters.put("NBR_ABSENCE", txtNbrAbsence.getText());
	                    parameters.put("PRIME_APAYER", textPrimeAPayer.getText());
	                    sommetowords = ConverterNumberToWords.converter(String.valueOf(AfficherFicheEmploye.this.txtTotalDu.getText()));
	                    parameters.put("somme", sommetowords);
		...
		JButton btnBulletinPaieSans = new JButton("Bulletin Paie Sans Prime");
		btnBulletinPaieSans.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				// List<FicheEmploye> listFicheEmploye=ficheEmployeDAO.findByDateSitutaion(dateDebutChooser.getDate(), dateFinChooser.getDate(), txtCNI.getText());
				 ...
				 BigDecimal salaireBrut=salaireDebase; 
				 BigDecimal retenu226=salaireBrut.multiply(mapParametre.get(PARAMETRE_CODE_TAUX_CNSS_226)).divide(new BigDecimal(100), 6,RoundingMode.HALF_UP);
				 BigDecimal retenu448=salaireBrut.multiply(mapParametre.get(PARAMETRE_CODE_TAUX_CNSS_448)).divide(new BigDecimal(100), 6,RoundingMode.HALF_UP);
				 BigDecimal totalRetenu=retenu448.add(retenu226);
				 BigDecimal netApayer=BigDecimal.ZERO; 
				...
				parameters.put("totalRetenu", totalRetenu.setScale(2, RoundingMode.FLOOR)+"");
				parameters.put("netApayer", netApayer.setScale(2, RoundingMode.FLOOR)+"");
				JasperUtils.imprimerBulletinPaieEmployeSansPrime(listFicheEmploye,parameters,ficheEmploye.getEmploye().getNom());
		...
		btnBulletinPaieSans.setIcon(new ImageIcon(AfficherFicheEmploye.class.getResource("/img/imprimer.png")));
		btnBulletinPaieSans.setBounds(881, 575, 181, 24);
		add(btnBulletinPaieSans);
		...
		intialiserTableau();
		  ...
		  String date="";
			...
			{	
				...
				coutHoraire=coutHeurs.multiply(ficheEmploye.getDelaiEmploye().setScale(4, RoundingMode.FLOOR));
				coutHeureSupp25=coutHeurs.multiply(new BigDecimal(1.25)).multiply(ficheEmploye.getHeureSupp25().setScale(4, RoundingMode.FLOOR));
				coutHeureSupp50=coutHeurs.multiply(new BigDecimal(1.5)).multiply(ficheEmploye.getHeureSupp50().setScale(4, RoundingMode.FLOOR));
				...
				Object []ligne={ficheEmploye.getId(),date,ficheEmploye.getEmploye().getNomafficher(),ficheEmploye.getDelaiEmploye().setScale(4, RoundingMode.HALF_UP),coutHoraire.setScale(4, RoundingMode.FLOOR),ficheEmploye.getRemise().setScale(4, RoundingMode.FLOOR),coutHeureSupp25.setScale(4, RoundingMode.FLOOR),coutHeureSupp50.setScale(4, RoundingMode.FLOOR),ficheEmploye.getHeureSupp25().setScale(4, RoundingMode.FLOOR),ficheEmploye.getHeureSupp50().setScale(4, RoundingMode.FLOOR)};
...
if(primeAnciennte!=null)
{
	Totalprimeanciennte=(primeAnciennte.getTaux().divide(new BigDecimal(100) , 4, RoundingMode.HALF_UP)).multiply(totalDu);
	...
	TauxAnciennete=primeAnciennte.getTaux();
			...
			txtTotalCout.setText(format.format(totalCout.setScale(4, RoundingMode.FLOOR)));
			txtTotalPrime.setText(totalPrime.setScale(4, RoundingMode.FLOOR)+""); 
			 txtPrimeAnciennete.setText(Totalprimeanciennte.setScale(4, RoundingMode.FLOOR)+"");
			 txtTauxAnciennete.setText(TauxAnciennete.setScale(4, RoundingMode.FLOOR)+"");
			//txtTotalDu.setText(totalDu+"");
			txtTotalCout25.setText(totalCout25.setScale(4, RoundingMode.FLOOR)+"");
			txtTotalCout50.setText(totalcout50.setScale(4, RoundingMode.FLOOR)+"");
			txtNbrAbsence.setText(compteurbsenseEmployernbr+"");
			txtTotalDelai.setText(totalDelai.setScale(4, RoundingMode.FLOOR)+"");
			

			calculertotalApayer(totalDu.setScale(4, RoundingMode.FLOOR), compteurbsenseEmployernbr, totalPrime);
...
//	primeAPayer=primeAPayer-(primeAPayer.remainder(BigDecimal.ONE));
	 ...
	 if( (primeAPayer.divide(new BigDecimal(50)).subtract(new BigDecimal(primeAPayer.divide(new BigDecimal(50)).intValue()) )).compareTo(new BigDecimal(0.5)) >=0  )
	 {
		 ...
		 {
			 primefinale= ((primeAPayer.divide(new BigDecimal(50)).intValue()+1)*50);
		 }else
		 {
			 primefinale= ((primeAPayer.divide(new BigDecimal(50)).intValue())*50); 
		 }
	...
	txtreduction.setText((reduction.setScale(6)+""));
	 DecimalFormat format = new DecimalFormat("#.00");
	txtTotalDu.setText(format.format(netapayer.setScale(2, RoundingMode.HALF_UP)));
...
List<FicheEmploye> calculeCoutEmploye(){
	...
	List<CoutHorsProdEnAttent> listCoutHorsProdEnAttent=coutHorsProdEnAttentDAO.ListHeursCoutHorsProdEnAttentByDate (dateDebutChooser.getDate(), dateFinChooser.getDate(), txtCNI.getText());
