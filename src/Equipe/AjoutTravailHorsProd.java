	...
	private FicheEmployeDAO ficheEmployeDAO;
	  ...
	  private Map< String, Employe> mapMatriculeEmploye = new HashMap<>();
		private Map< String, Employe> mapNomEmploye = new HashMap<>();
		 ...
		 private JTextField txtQuantiteTrier;
		 ...
		 JComboBox comboMP = new JComboBox();
		  ...
		  private Map< String, MatierePremier> mapCodeMatierePremier = new HashMap<>();
		  private List<DetailMPTrierHorsProduction> listDetailMPTrierHorsproduction= new ArrayList<DetailMPTrierHorsProduction>();
        ...
        try{
            ...
            imgInit= new ImageIcon(this.getClass().getResource("/img/init.png"));
             ...
             imgValider = new ImageIcon(this.getClass().getResource("/img/valider.png"));
				  		   ...
				  		   layeredPane.setBounds(6, 273, 1119, 240);
				  		   add(layeredPane);
...
if(coutHorsProdEnAttentTmp!=null)
{
	coutHorsProdEnAttentDAO.delete(coutHorsProdEnAttentTmp);
				  		   ...
				  		   layeredPane.add(btnSupprimerLigne);
				  		   		...
				  		   		scrollPane.setBounds(22, 0, 1004, 229);
	...
	btnValiderAjoutMachine.setBounds(472, 839, 148, 26);
	add(btnValiderAjoutMachine);
	btnValiderAjoutMachine.setIcon(imgValider);
	
	JLayeredPane layeredPane_2 = new JLayeredPane();
	layeredPane_2.setBorder(new MatteBorder(0, 1, 1, 1, (Color) Color.LIGHT_GRAY));
	layeredPane_2.setBounds(6, 606, 1119, 222);
	add(layeredPane_2);
	
	JButton button = new JButton("");
	button.setBounds(1049, 64, 60, 26);
	layeredPane_2.add(button);
	
	JScrollPane scrollPane_1 = new JScrollPane((Component) null);
	scrollPane_1.setBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null));
	scrollPane_1.setBounds(21, 11, 1004, 201);
	...
	txtCodeMp.addKeyListener(new KeyAdapter() {
		...
		public void keyPressed(KeyEvent e) {
			...
			if(e.getKeyCode()==e.VK_ENTER)
      		{
				MatierePremier mp=mapCodeMatierePremier.get(txtCodeMp.getText().toUpperCase());
				if(mp!=null)
				{
	...
	txtCodeMp.setBounds(83, 535, 109, 26);
	add(txtCodeMp);
	
	JLabel label_1 = new JLabel("MP :");
	label_1.setFont(new Font("Tahoma", Font.PLAIN, 11));
	label_1.setBounds(211, 537, 37, 24);
	add(label_1);
	...
	comboMP.setBounds(245, 537, 359, 24);
	add(comboMP);
	
	JLabel lblQuantiteTrier = new JLabel("Quantite Trier :");
	lblQuantiteTrier.setFont(new Font("Tahoma", Font.PLAIN, 12));
	lblQuantiteTrier.setBounds(629, 535, 95, 26);
	add(lblQuantiteTrier);
	
	txtQuantiteTrier = new JTextField();
	txtQuantiteTrier.setColumns(10);
	txtQuantiteTrier.setBounds(734, 536, 205, 26);
	add(txtQuantiteTrier);
	...
	for(int i=0;i<listMatierePremiere.size();i++)
	{
		...
		mapCodeMatierePremier.put(matierePremier.getCode(), matierePremier);
	}
	...
	button_1.addActionListener(new ActionListener() {
		...
		if(txtQuantiteTrier.getText().trim().isEmpty()==true || txtQuantiteTrier.getText().trim().equals(""))
		{
			JOptionPane.showMessageDialog(null, "Veuillez Entrer la Quantite Trier SVP","Erreur",JOptionPane.ERROR_MESSAGE);
			return;
		}
		
		try {
			
			if(new BigDecimal(txtQuantiteTrier.getText()).compareTo(BigDecimal.ZERO)>0)
			{
			...
			{
				JOptionPane.showMessageDialog(null, "La Quantite doit etre superieur a Zero SVP","Erreur",JOptionPane.ERROR_MESSAGE);
				return;
			}
	...
	button_1.setBounds(353, 581, 89, 26);
	add(button_1);
	...
	button_2.setBounds(452, 581, 95, 26);
	add(button_2);
	}
