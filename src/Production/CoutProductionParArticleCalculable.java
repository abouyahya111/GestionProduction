...
public class CoutProductionParArticleCalculable extends JLayeredPane implements Constantes{
	...
	private List<Object[]> listObjetTransfertMPPF =new ArrayList<Object[]>();
	 ...
	 */
	public CoutProductionParArticleCalculable() {
		setOpaque(true);
...
void CalculerCoutProductionParArticle()
{
	...
	Depot depot=mapDepot.get(combodepot.getSelectedItem());
		Articles articles=mapLibelleAricle.get(comboBoxArticle.getSelectedItem());
	listObjetQuantiteReelMPParArticle.clear();
	listObjetTransfertMPPF.clear();
		listObjetQuantiteReelMPParArticle=coutMPDAO.listeSumQuantiteReelParArticle (dateDebutChooser.getDate(), dateFinChooser.getDate(),Constantes.ETAT_OF_TERMINER,depot.getCode(),articles);
		listObjetTransfertMPPF=detailTransferProduitFiniDAO.SommeTotalMontantTransfertMPPFParArticle(Constantes.ETAT_TRANSFER_STOCK_ENTRER_MP);
  	...
  	for(int s=0;s<listEtatCoutProduction.size();s++)
  	{
		...
		{
			  coutTotalPrimeEmployeGenerique= BigDecimal.ZERO;
			...
			for(int k=0;k<listCoutMP.size();k++)
			{
				...
				coutTotalDechetMP=coutTotalDechetMP.add((listCoutMP.get(k).getQuantDechet().multiply(listCoutMP.get(k).getPrixUnitaire())).setScale(12, RoundingMode.HALF_UP));
				if(!listCoutMP.get(k).getMatierePremier().getCategorieMp().getSubCategorieMp().getCode().equals(Constantes.SOUS_CATEGORIE_MATIERE_PREMIERE_CADEAU))
				{
					...
					coutTotalOffreMP=coutTotalOffreMP.add((listCoutMP.get(k).getQuantiteOffre().multiply(listCoutMP.get(k).getPrixUnitaire())).setScale(12, RoundingMode.HALF_UP));
				}
				...
				coutTotalCoutQuantiteConsommeMP=coutTotalCoutQuantiteConsommeMP.add((quantiteConsommeSoustraireQuantitePlus.multiply(listCoutMP.get(k).getPrixUnitaire())).setScale(12, RoundingMode.HALF_UP));
				coutTotalDechetFournisseurMP=coutTotalDechetFournisseurMP.add((listCoutMP.get(k).getQuantDechetFournisseur().multiply(listCoutMP.get(k).getPrixUnitaire())).setScale(12, RoundingMode.HALF_UP));
				coutTotalManquanteMP=coutTotalManquanteMP.add((listCoutMP.get(k).getQuantiteManquante().multiply(listCoutMP.get(k).getPrixUnitaire())).setScale(12, RoundingMode.HALF_UP));
			...
			for(int i=0;i<listEmployeGesnerique.size();i++)
			{
				...
				{
					coutTotalPrimeEmployeGenerique= coutTotalPrimeEmployeGenerique.add((((listEmployeGesnerique.get(i).getRemise().divide(new BigDecimal(8), 6, RoundingMode.HALF_UP)).multiply(listEmployeGesnerique.get(i).getDelaiEmploye())).divide(listEmployeGesnerique.get(i).getTotalHeuresProduction(), 6, RoundingMode.HALF_UP)).multiply(production.getNbreHeure())) ;
				}
			}
			
			TotalCoutEmployerResponsableParProduction=TotalCoutEmployerResponsableParProduction.add(((coutTotalEmployeGenerique.add(coutTotalSupp25EmployeGenerique.add(coutTotalSupp50EmployeGenerique))).divide(TotalHeuresProduction, 6, RoundingMode.HALF_UP)).multiply(production.getNbreHeure())) ;
			TotalCoutEmployerResponsableParProduction=TotalCoutEmployerResponsableParProduction.add(coutTotalPrimeEmployeGenerique) ;
		...
		etatCoutProduction.setCoutMP(coutTotalCoutQuantiteConsommeMP.add(coutTotalDechetMP.add(coutTotalDechetFournisseurMP).add(coutTotalOffreMP.add(coutTotalManquanteMP))));
		...
		etatCoutProduction.setQuantiteRealiser(QuantiteReel);
		etatCoutProduction.setTotal((etatCoutProduction.getCoutMP().setScale(4, RoundingMode.HALF_UP).add(etatCoutProduction.getCoutEmployeEmballage().setScale(4, RoundingMode.HALF_UP).add(etatCoutProduction.getCoutEmployeGenerique().setScale(4, RoundingMode.HALF_UP).add(etatCoutProduction.getCoutEmployeProduction().setScale(4, RoundingMode.HALF_UP))))).setScale(4, RoundingMode.HALF_UP));
		etatCoutProduction.setCout((etatCoutProduction.getTotal()) .divide(etatCoutProduction.getQuantiteRealiser(), 4, RoundingMode.HALF_UP));	
	...
	for(int j=0;j<listObjetTransfertMPPF.size();j++)
	{
		 Object[] object=listObjetTransfertMPPF.get(j);	
		if(object[0]!=null)
		{
			Articles article=(Articles)object[0];
			...
			etatCoutProduction.setArticle(article);
			etatCoutProduction.setCoutMP((BigDecimal) object[2]);
			etatCoutProduction.setCoutEmployeGenerique(BigDecimal.ZERO);
			etatCoutProduction.setCoutEmployeProduction (BigDecimal.ZERO);
			etatCoutProduction.setCoutEmployeEmballage(BigDecimal.ZERO);
			etatCoutProduction.setQuantiteRealiser((BigDecimal) object[1]);			
			etatCoutProduction.setTotal((BigDecimal) object[2]);
			if(etatCoutProduction.getQuantiteRealiser().compareTo(BigDecimal.ZERO)!=0)
			{
				etatCoutProduction.setCout(etatCoutProduction.getTotal().divide(etatCoutProduction.getQuantiteRealiser(), 4, RoundingMode.HALF_UP));
			}
			...
			coutTotalConsommeMPTmp=coutTotalConsommeMPTmp.add(etatCoutProduction.getCoutMP());
	...
	void afficher_tableMP(List<EtatCoutProduction> listCoutArticlePF)
	{
	intialiserTableau();
	...
	dfDecimal.setGroupingUsed(true);
		...
		{	
			EtatCoutProduction coutArticlePF=listCoutArticlePF.get(i);
			...
			TotalcoutGenerique=TotalcoutGenerique.add(coutArticlePF.getCoutEmployeGenerique());
			TotalcoutProduction=TotalcoutProduction.add(coutArticlePF.getCoutEmployeProduction());
			TotalcoutEmballage=TotalcoutEmballage.add(coutArticlePF.getCoutEmployeEmballage());
			...
			TotalRealiser=TotalRealiser.add(coutArticlePF.getQuantiteRealiser());
					 ...
					 Object []ligne={coutArticlePF.getArticle().getCodeArticle(),coutArticlePF.getArticle().getLiblle(),dfDecimal.format(coutArticlePF.getCoutMP().setScale(4, RoundingMode.HALF_UP)),dfDecimal.format(coutArticlePF.getCoutEmployeGenerique().setScale(4, RoundingMode.HALF_UP)),dfDecimal.format(coutArticlePF.getCoutEmployeProduction().setScale(4, RoundingMode.HALF_UP)),dfDecimal.format(coutArticlePF.getCoutEmployeEmballage().setScale(4, RoundingMode.HALF_UP)),dfDecimal.format(coutArticlePF.getTotal().setScale(4, RoundingMode.HALF_UP)),dfDecimal.format(coutArticlePF.getQuantiteRealiser().setScale(4, RoundingMode.HALF_UP)),dfDecimal.format(coutArticlePF.getCout().setScale(4, RoundingMode.HALF_UP))};
		...
		labelTotalrealiser.setText("Total RÈaliser : "+dfDecimal.format(TotalRealiser.setScale(4, RoundingMode.HALF_UP)));
