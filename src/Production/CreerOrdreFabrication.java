		 ...
		 private Map< String, BigDecimal> mapQuantiteOffreVariant = new HashMap<>();
			private dao.daoManager.OffreProductionDAO OffreProductionDAO;
			 ...
			 private List<FormeParBox> listFormeParBox =new ArrayList<FormeParBox>();
				...
				private List<SituationDesEmployeesAbsents> listAutorisationDesAbsents=new ArrayList<SituationDesEmployeesAbsents>();
				  		  		...
				  		  		JOptionPane.showMessageDialog(null, "Il faut remplir la quantité!", "Attention", JOptionPane.INFORMATION_MESSAGE);
					  		  		...
					  		  		JOptionPane.showMessageDialog(null, "Il faut choisir un magasin Source!", "Attention", JOptionPane.INFORMATION_MESSAGE);
						  		  		...
						  		  		JOptionPane.showMessageDialog(null, "Il faut choisir un magasin Production!", "Attention", JOptionPane.INFORMATION_MESSAGE);
							  		  		...
							  		  		listAutorisationDesAbsents.clear(); 
							  		  		 
							  		  		listProductionsTerminer=productionDAO.LesProductionTerminerParDateParDepotGroupByDate(dateDebutChooser.getDate(), Constantes.ETAT_OF_TERMINER, depot.getCode()) ;
							  		  	if(listProductionsTerminer.size()!=0)
							  		  	{
							  		  	Production productionTmp=listProductionsTerminer.get(listProductionsTerminer.size()-1)	;
							  		  	
							  		  	listAutorisationDesAbsents=VerifierAbsentEmploye(productionTmp.getDate());
							  		  	
							  		  	
							  		  	}
							  		  		
							  		  		if(listAutorisationDesAbsents.size()!=0)
							  		  		{
							  		  			
							  		  		JOptionPane.showMessageDialog(null, "Veuillez Valider l'Autorisation Des Absences Des date Ulterieur SVP");
					  		  				 return;
...
return;
				  		     	...
				  		     	TypeOffre typeOffre=mapTypeOffre.get(comboTypeOffre.getSelectedItem());
				  		     			...
				  		     			production.setNbrePersonneTravailler(BigDecimal.ZERO);
				  		     			if(!comboVariant.getSelectedItem().equals(Constantes.OFFRE_VARIANT))
				  		     			{
					        ...
					        util.Utils.copycollercell(textField);
				  		     ...
				  		     comboMachine.addItem("");
				  		       i=0;
				  		     	...
				  		     	layeredPane.setBounds(20, 400, 1376, 161);
				  		     	add(layeredPane);
 ...
 boolean selected=false;
	...
	void ChargerSousCategorieBox()
	{
		...
		for(int t=0;t<listSousCategorie.size();t++)
		{
			...
			if(listSousCategorie.get(t).getCode().equals(SOUS_CATEGORIE_MATIERE_PREMIERE_BOX) || listSousCategorie.get(t).getCode().equals(SOUS_CATEGORIE_MATIERE_PREMIERE_BOX_METALIQUE) || listSousCategorie.get(t).getCode().equals(SOUS_CATEGORIE_MATIERE_PREMIERE_BOITE) )
			{
	...
	modeleMPOffre =new DefaultTableModel(
		     	...
		     	public boolean isCellEditable(int row, int column) {
    				...
    				if(typeOffre!=null)
    				{
		     		...
		     		{
		     			return false;
		     		}else if(column==3 && typeOffre.getTypeOffre().equals(Constantes.TYPE_OFFRE_AUTRES) && comboVariant.getSelectedItem().toString().equals(Constantes.OFFRE_VARIANT)  && (boolean)(tableOffre.getValueAt(row, 4))==true )
		     		{
		     			return false;
		     		}
	...
	for(int i=0;i<lisDetailEstimation.size();i++){
		...
		quantiteExistante=BigDecimal.ZERO;
		 ...
		 stockMPQauantiteManquante=new StockMP();
			...
			if(detailEstimation.getMatierePremier().getCode().equals("MP_1314"))
			{
				System.out.println(detailEstimation.getMatierePremier().getCode());
			}
					...
					if( detailEstimation.getMatierePremier().getCategorieMp().getSubCategorieMp().getCode().equals(SOUS_CATEGORIE_MATIERE_PREMIERE_BOX))
					{
						...
						if(NombreBox.compareTo(BigDecimal.ONE)==1)
						{
							...
							}else
							{
								...
								if(mapQuantiteRestereMatierePremier.get(detailEstimation.getMatierePremier().getCategorieMp().getSubCategorieMp().getCode().trim())!=null)
								{
									quantiteTotal=mapQuantiteRestereMatierePremier.get(detailEstimation.getMatierePremier().getCategorieMp().getSubCategorieMp().getCode().trim());
				...
				for(int d=0;d<listEtatStockMPAfficherMagasinStockage.size();d++)
				{
					if(listEtatStockMPAfficherMagasinStockage.get(d).getMp().getId()==detailEstimation.getMatierePremier().getId())
					{
					 ...
					 if(listEtatStockMPAfficherMagasinStockage.get(d).getQuantiteFinale().compareTo(quantiteTotal)>=0)
						...
						for(int f=0;f<lisDetailEstimation.size();f++)
						{
							if(BoxClientExiste==true)
							{
								
								if(lisDetailEstimation.get(f).getMatierePremier().getCode().equals(listEtatStockMPAfficherMagasinStockage.get(d).getMp().getCode()+"C"))
								{
							...
							}else
							{
								...
								if(lisDetailEstimation.get(f).getMatierePremier().getCode().equals(listEtatStockMPAfficherMagasinStockage.get(d).getMp().getCode()))
								{
									rest=quantiteTotal.subtract(listEtatStockMPAfficherMagasinStockage.get(d).getQuantiteFinale());
									
									
									mapQuantiteRestereMatierePremier.put(listEtatStockMPAfficherMagasinStockage.get(d).getMp().getCategorieMp().getSubCategorieMp().getCode(), rest);
...
int nbrErreur=0;
					...
					for(int i=0;i<tableOffre.getRowCount();i++)
					{
						...
						if(selected==true)
						{
							...
							if(!tableOffre.getValueAt(i, 3).equals(""))
							{
								...
								if(comboVariant.getSelectedItem().equals(Constantes.OFFRE_NORMAL))
								{	
			...
			for(int t=0;t<listDetailEstimationOffreMPSelectionner.size();t++)
			{
				...
				if(detailEstimationPromoTmp.getMatierePremiere().getId()==detailEstimationOffreSelectionner.getMatierePremiere().getId())
				{
					...
					if(detailEstimationPromoTmp.getMatierePremiere().getCategorieMp().getSubCategorieMp().getCode().equals(SOUS_CATEGORIE_MATIERE_PREMIERE_THE))
					{
						...
						{
							MPexiste=true;
							if(detailEstimationOffreSelectionner.getQuantite()!=null)
							{
								detailEstimationPromoTmp.setQuantite(detailEstimationOffreSelectionner.getQuantite());
								detailEstimationPromoDAO.edit(detailEstimationPromoTmp);
							}
    ...
    List<SituationDesEmployeesAbsents> VerifierAbsentEmploye( Date dateProduction){
    	...
    	List<EmployeRepos> listEmployeRepos=new ArrayList<EmployeRepos>();
    	  List<CoutHorsProdEnAttent> listCoutHorsProductionEnAttente = new ArrayList<CoutHorsProdEnAttent>();
    	      ...
    	      listEmployeRepos=employeReposDAO.findByBepotByDateGroupByEmployee(dateProduction, depot);
    		 listDetailProduction =detailProductionDAO.ListHeursDetailProductionParDepotparJourGroupByEmployer(dateProduction, depot.getId(),Constantes.ETAT_OF_TERMINER) ;
    	     listDetailProdGenerique=detailProdGenDAO.ListHeursDetailProdGenParDepotParJourGroupByEmployee(dateProduction, depot.getId(),Constantes.ETAT_OF_TERMINER);
    		 listDetailResponsableProd=detailProdResDAO.ListHeursDetailResponsableProdParDepotParJourGroupByEmployee(dateProduction, depot.getId());
    		 listDetailProductionMP=detailProductionMPDAO.ListHeursDetailProductionMPParDepotParJourGroupByEmployee(dateProduction, depot.getId(),Constantes.ETAT_OF_TERMINER);
    		 listCoutHorsProductionEnAttente=coutHorsProdEnAttentDAO.findBydateByDepotGroupByEmploye(dateProduction, depot);
