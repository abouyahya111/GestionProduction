		 ...
		 private ParametreDAO parametreDAO;
		  private CoutMPDAO    CoutMPDAO;
			...
			JRadioButton radioPlus = new JRadioButton("Plus");
			  ...
			  private JButton btnNombreCartonParOffre;
       ...
       System.exit(0);
        ...
        codeDepot= AuthentificationView.utilisateur.getCodeDepot();
				  		...
				  		initialiserTableOffreVariante();
				  		 ...
				  		 btnImprimer.setBounds(639, 837, 133, 24);
				  		 add(btnImprimer);
				  				...
				  				if(production!=null){
				  					...
				  					if(production.getStatut().equals(Constantes.ETAT_OF_LANCER))
				  					{
				  						...
				  						if(production.getNbrePersonneTravailler()!=null)
				  						{
				  							...
				  							if(production.getNbrePersonneTravailler().compareTo(BigDecimal.ZERO)<=0)
				  							{
				  								JOptionPane.showMessageDialog(null, "Veuillez Entrer Le Nombre Des Personnes Travailler dans Cette production SVP");
					  							return;
				  							}
...
if(listDetailResponsableProdTmpVerification.size()!=0)
{
	for(int i=0;i<listDetailResponsableProdTmpVerification.size();i++)
		{
		if(listDetailResponsableProdTmpVerification.get(i).getTotalHeuresProduction()!=null)
		{
			if(listDetailResponsableProdTmpVerification.get(i).getTotalHeuresProduction().compareTo(BigDecimal.ZERO)!=0)
			{
				
				totalHeuresProduction=listDetailResponsableProdTmpVerification.get(i).getTotalHeuresProduction();
			}
		}
...
{
	JOptionPane.showMessageDialog(null, "Total heures de Production  Déclarer Dans l'equipe Générique égale à 0 , Veuillez Modifier Le SVP","Errgeur",JOptionPane.ERROR_MESSAGE);
	return;
}
...
if(numberproduction!=0 && nbrProd!=0)
{
	...
	}else if(nbrProd -numberproduction==1)
	{
		BigDecimal heuresGlobalProduction=BigDecimal.ZERO;
		
		listProductionsTerminer=productionDAO.LesProductionTerminerParDateSpecifierParDepotGroupByDate(production.getDate(),  Constantes.ETAT_OF_TERMINER, production.getCodeDepot());
		
		for(int j=0;j<listProductionsTerminer.size();j++)
		{
			heuresGlobalProduction=heuresGlobalProduction.add(listProductionsTerminer.get(j).getNbreHeure());
		}
		heuresGlobalProduction=heuresGlobalProduction.add(new BigDecimal(txtPrixServiceProd.getText()));
		
		if(totalHeuresProduction.compareTo(heuresGlobalProduction)!=0)
		{
			
			JOptionPane.showMessageDialog(null, "Total Heures Déclarer Dans l'Equipe Generique est different au Total Delai Production ","Errgeur",JOptionPane.ERROR_MESSAGE);
			return;
	 ...
	 /////////////////////////////////////////////////////////  Importer Les Heures Employee Production     ///////////////////////////////////////////////////////////////////////////////////
				  					/*
				  					 ErreurImporterHeure=false;
				  					AjouterLesEmployee();
				  					if(ErreurImporterHeure==true)
				  					{
				  						return;
				  					}
				  					*/
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////			  					
					  						 ...
					  						 for(int j=0;j<listCoutMPTmp.size();j++){
					  							CoutMP  coutMP=listCoutMPTmp.get(j); 
					  							 if( coutMP.getMatierePremier().getCategorieMp().getSubCategorieMp().getCode().equals(SOUS_CATEGORIE_MATIERE_PREMIERE_CARTON)){
						  							...
						  							{
                                                        if(production.getTyoeOffre().getTypeOffre().equals(Constantes.TYPE_OFFRE_PF) || production.getTyoeOffre().getTypeOffre().equals(Constantes.TYPE_OFFRE_AUTRES_PF) || production.getTyoeOffre().getTypeOffre().equals(Constantes.TYPE_OFFRE_AUTRES))
						  									
							  							{
                                                        	...
                                                        	}else if(production.getTyoeOffre().getTypeOffre().equals(Constantes.TYPE_OFFRE_AUTRES) )
                                                        	{
					  						 ...
					  						 if(NBRPFMixte>=1)
					  						 {
					  							...
					  							if(production.getOffreVariant()!=null)
		  										{
					  								if(production.getOffreVariant().equals(Constantes.CODE_OUI))
		  											{
					  									...
					  									if(production.getTyoeOffre().getTypeOffre().contentEquals(Constantes.TYPE_OFFRE_AUTRES))
										  				{
					  										BigDecimal nombreCartonPourOffreVariante=BigDecimal.ZERO;
					  										
					  										listNombreCartonOfferVariante=nombreCartonOfferVariantDAO.findByProduction(production);
					  										
					  										for(int j=0;j<listNombreCartonOfferVariante.size();j++)
					  										{
					  											NombreCartonOffreVariante nombreCartonOffreVariante=listNombreCartonOfferVariante.get(j);
					  											listDetailNombreCartonOfferVariante=detailNombreCartonOfferVariantDAO.findDetailNombreCartonOffreVarianteByNombreCartonOffreVariant(nombreCartonOffreVariante);
					  											
					  											  for(int d=0;d<listDetailNombreCartonOfferVariante.size();d++)
					  											  {
					  												  DetailNombreCartonOffreVariante detailNombreCartonOffreVariante=listDetailNombreCartonOfferVariante.get(d);
					  													quantiteTotalOffreCalculer=quantiteTotalOffreCalculer.add(detailNombreCartonOffreVariante.getEstimation().multiply(nombreCartonOffreVariante.getNombreCarton()));
 
					  												  
					  											  }
					  											  if(listDetailNombreCartonOfferVariante.size()!=-1)
					  											  {
					  												nombreCartonPourOffreVariante=nombreCartonPourOffreVariante.add(nombreCartonOffreVariante.getNombreCarton()) ;
					  											  }
					  										...
					  										{
					  											btnNombreCartonParOffre.setVisible(false);
					  										}
				  		...
				  		btnChercherOF.setIcon(new ImageIcon(CreerOrdreFabrication.class.getResource("/img/chercher.png")));
				  		 ...
				  		 add(btnChercherOF);
				  		    ...
				  		    btnTerminerOF.setBounds(203, 837, 112, 24);
				  		    add(btnTerminerOF);
...

				  		     	...
				  		     	/*	
				  		     		...
				  		     		*/
					  					...
					  					coutTotal=production.getCoutTotal();
	    ...
	    StatistiqueFinanciaireMP statistiqueFinanciaireMPTmp=listStatistiqueFinanciereMP.get(listStatistiqueFinanciereMP.size()-1);
			  		     			...
			  		     			statistiqueFinanciaireMP.setEtatTransfer(Constantes.ETAT_OF_TERMINER);
	...
	//	scrollPane_1.setViewportView(tableEmploye);
		...
		add(titledSeparator_1);
				  		    ...
				  		    btnValiderSaisie.setBounds(424, 837, 102, 24);
				  		    ...
				  		    btnAnnuler.setBounds(325, 838, 89, 23);
				  		    add(btnAnnuler);
				  		  btnAnnuler.setVisible(false);
				  		    ...
				  		    btnFactureService.setBounds(782, 837, 112, 23);
				  		    ...
				  		    btnSaisirDelaiEmploy_1.setBounds(8, 839, 185, 23);
				  		  btnSaisirDelaiEmploy_1.setVisible(false);
				  		    ...
				  		    btnSaisirDelaiEmploy.setBounds(909, 837, 185, 23);
				  		  btnSaisirDelaiEmploy.setVisible(false);
				  		   ...
				  		   codeArticle.setColumns(10);
				  		     ...
				  		     scrollPane_3.setBounds(970, 641, 887, 145);
				  		     add(scrollPane_3);
					 ...
					 */
				  		   ...
				  		   supprimeremployergen.setVisible(false);
				  		     ...
				  		     btnExporterExcel.setBounds(536, 838, 102, 24);
				  		   ...
				  		   checkabsentgenerique.setVisible(false);
				  		     ...
				  		     checkEnPaneemballage.setBounds(1701, 325, 71, 23);
 ...
 if(checkEnpaneproduction.isSelected()==true)
 {
	 enpanne=true;
						...
						checkretardproduction.setBounds(886, 334, 71, 24);
							  			...
							  			if(!txtNumOF.getSelectedItem().equals(""))
							  			{
								  			...
								  			}else
								  			{
								  				...
								  				if(productionTmp.getStatut().equals(Constantes.ETAT_OF_LANCER))
								  				{
								  					...
								  					if(productionTmp.getNbrePersonneTravailler()!=null)
										  			{
										  				txtNombrePersonneTravailler.setText(productionTmp.getNbrePersonneTravailler()+"");
										  			}
								  					...
								  					if(productionTmp.getOffreVariant()!=null)
								  					{
								  						...
								  						if(productionTmp.getTyoeOffre()!=null)
											  			{
												  			...
												  			}else
												  			{
												  				...
												  				if(productionTmp.getTyoeOffre().getTypeOffre().contentEquals(Constantes.TYPE_OFFRE_AUTRES))
												  				{
												  					
												  					if(productionTmp.getOffreVariant().equals(Constantes.CODE_OUI))
											  						{
												  						
												  					 btnNombreCartonParOffre.setVisible(true);
						...
						btnReglerStockPf.setBounds(1162, 837, 112, 24);
						...
						labelTotalEquipeEmballage.setBounds(958, 517, 420, 26);
						...
						labelTotalEquipeGenerique.setBounds(963, 797, 437, 26);
						add(labelTotalEquipeGenerique);
						
						  btnNombreCartonParOffre = new JButton("Nombre Carton Par Offre");
						  btnNombreCartonParOffre.addActionListener(new ActionListener() {
						  	public void actionPerformed(ActionEvent e) {
				  						...
				  						if(productionTmp.getTyoeOffre()!=null)
							  			{
								  				...
								  				if(productionTmp.getTyoeOffre().getTypeOffre().contentEquals(Constantes.TYPE_OFFRE_AUTRES))
								  				{
								  					...
								  					if(productionTmp.getOffreVariant().equals(Constantes.CODE_OUI))
							  						{
								  						...
								  						if(txtQuantiteRealise.getText().equals("") || txtPrixServiceProd.getText().equals(""))
								  						{
								  							JOptionPane.showMessageDialog(null, "Veuillez Entrer La Quantite Reel Et L'Heure De Production SVP");
								  							return;
								  						}
								  						
								  						if(productionTmp.getId()>0)
									  		    		{
									  		    			JFrame popupJFrame = new AjouterArticlesOffreVariantAuCarton(productionTmp,txtQuantiteRealise.getText(),txtPrixServiceProd.getText());
											  		    	   popupJFrame.setVisible(true);
									  		    		}else
									  		    		{
									  		    			JOptionPane.showMessageDialog(null, "Ordre de fabrication introuvable !!", "Erreur", JOptionPane.ERROR_MESSAGE);
						...
						btnNombreCartonParOffre.setHorizontalAlignment(SwingConstants.LEADING);
						btnNombreCartonParOffre.setFont(new Font("Tahoma", Font.PLAIN, 11));
						btnNombreCartonParOffre.setBounds(1725, 8, 189, 23);
						add(btnNombreCartonParOffre);
						...
						Production production= listProduction.get(i);
						 ...
						 txtNombrePersonneTravailler.setEditable(false);
						 txtNombrePersonneTravailler.setColumns(10);
						 txtNombrePersonneTravailler.setBounds(1325, 9, 83, 26);
			...
			{	
				...
				// String strEcart = nf.format(ecart);
				 String strEcart = NumberFormat.getNumberInstance(Locale.FRANCE).format(ecart);	 
					...
					{
						System.out.println(coutMP.getMatierePremier().getCode()+ " "+coutMP.getQuantite() + " - "+coutMP.getQuantConsomme() + " = "+ecartConsomme.setScale(2, BigDecimal.ROUND_HALF_DOWN));
	...
	int nbrPersonnesTravailler=0;
		  ...
		  int i=0;
			...
			{	
				...
				Object []ligne={detailProduction.getEmploye().getId(),detailProduction.getEmploye().getMatricule(),detailProduction.getEmploye().getNomafficher(),delai,heureSupp25,heureSupp50,absent,sortie,retard,panne};
	...
	int nbrPersonnesTravailler=0;
	  int i=0;
		...
		{	
			...
			Object []ligne={detailProdGen.getEmploye().getId(),detailProdGen.getEmploye().getMatricule(),detailProdGen.getEmploye().getNomafficher(),delai,heureSupp25,heureSupp50,absent,sortie,retard,panne};
...
}else if( production.getTyoeOffre().getTypeOffre().equals(Constantes.TYPE_OFFRE_AUTRES))
{
	...
	if(production.getOffreVariant()!=null)
	{
		...
		if(production.getOffreVariant().equals(Constantes.CODE_OUI))
		{
			...
			BigDecimal quantiteOffreCalculer=BigDecimal.ZERO;
			...
			listNombreCartonOfferVariante=nombreCartonOfferVariantDAO.findByProduction(production);
				...
				for(int j=0;j<listNombreCartonOfferVariante.size();j++)
				{
					...
					listDetailNombreCartonOfferVariante=detailNombreCartonOfferVariantDAO.findDetailNombreCartonOffreVarianteByNombreCartonOffreVariant(nombreCartonOffreVariante);
					  ...
					  for(int d=0;d<listDetailNombreCartonOfferVariante.size();d++)
					  {
						  ...
						  if(detailNombreCartonOffreVariante.getMatierePremier().getId()==coutMP.getMatierePremier().getId())
						  {
							  
						quantiteOffreCalculer=quantiteOffreCalculer.add(detailNombreCartonOffreVariante.getEstimation().multiply(nombreCartonOffreVariante.getNombreCarton()));  
				...
				if(coutMP.getQuantConsomme().compareTo(quantiteOffreCalculer.setScale(0, RoundingMode.FLOOR))!=0)
				{
...
detailTransferStockMPDAO.ViderSession();
	...
	for(int i=0;i<listCoutMP.size();i++){ 
		...
		*/
		...
		prixMP=(quantiteConsomme.subtract(coutMP.getQuantiteManquanteFrPlus())).multiply(prixUnitaire);
...
void initialiserTableauEmploye(){
	modeleEmploye =new DefaultTableModel(
		     	...
		     	new String[] {
		     			"ID","Matricule","Nom", "Délai Travaillé", "H Supp 25%", "H Supp 50%", "Absent","Sortie","Retard","Panne"
		     	}
		     ) {
     	boolean[] columnEditables = new boolean[] {
     	...
     	Class[] columnTypes = new Class[] {
     			String.class,String.class,String.class,BigDecimal.class,BigDecimal.class,BigDecimal.class, Boolean.class, Boolean.class,Boolean.class,Boolean.class
			};
     	...
     	public boolean isCellEditable(int row, int column) {
     		...
     		if(column==3 || column==4 || column==5 || column==6 || column==7 || column==8 || column==9)
     		{
     			return false;
     		}else
     		return columnEditables[column];
