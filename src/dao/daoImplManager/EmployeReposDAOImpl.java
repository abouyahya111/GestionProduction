...
public class EmployeReposDAOImpl implements EmployeReposDAO {
	...
	public List<EmployeRepos> findByBepotByDateGroupByEmployee(Date datedebut, Depot depot) {
		Query query= session.createQuery("select c from EmployeRepos c where dateSituation =:datedebut and  employe.depot.code=:depot group by  employe");
		query.setParameter("datedebut", datedebut);
		query.setParameter("depot", depot.getCode());
		//query.setParameter("actif", true);
		return query.list();
