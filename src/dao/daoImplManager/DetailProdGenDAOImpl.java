...
public class DetailProdGenDAOImpl implements DetailProdGenDAO {
	...
	public void delete(DetailProdGen DetailProdGen) {
		
		session.beginTransaction();
	//	DetailProdGen p= findById(id);
		session.delete(DetailProdGen);
		session.getTransaction().commit();
	...
	public List<DetailProdGen> ListHeursDetailProdGenParDepotParJourGroupByEmployee(Date dateDebut, int depot ,String statut) {
 
		Query query=null;
		  		
				query= session.createQuery("select c from DetailProdGen c where c.productionGen.date =:dateDebut  and c.employe.depot.id=:depot and c.productionGen.statut=:statut group by c.employe");
				query.setParameter("dateDebut", dateDebut);
				query.setParameter("depot", depot);
				query.setParameter("statut", statut);
