...
public List<Production> listeProductionEtatLancerAndNombreEmployerTravaillerEgaleAzero(String lancer,String depot) {
		
		Query query= session.createQuery("select p from Production p where  statut =:statutlancer and code_depot=:depot and nbrePersonneTravailler=0");		
	...
	public List<Production> LesProductionTerminerParDateParDepotGroupByDate(Date date , String etat,String codedepot) {
		...
		int id =0;
		Long l = null;
		Query query= session.createQuery("select c from Production c where c.date !=:date and c.statut =:etat and codeDepot=:codedepot group by c.date order by c.date ");
	...
	@Override
	public List<Production> LesProductionTerminerParDateSpecifierParDepotGroupByDate(Date date , String etat,String codedepot) {
		// TODO Auto-generated method stub
		int id =0;
		Long l = null;
	...
	@Override
	public List<Object[]> SommeTotalMontantProduction(String etat) {
		// TODO Auto-generated method stub
		Query query=null;
		
			
			query= session.createQuery("select p.id, sum(p.coutTotal)/sum(p.quantiteReel) * sum(p.quantiteReel) FROM Production p where p.statut =:statut");
