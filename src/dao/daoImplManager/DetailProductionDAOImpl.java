...
public class DetailProductionDAOImpl implements DetailProductionDAO {
	...
	public void delete(DetailProduction DetailProduction) {
		
		session.beginTransaction();
		//DetailProduction p= findById(id);
		session.delete(DetailProduction);
		session.getTransaction().commit();
	...
	public DetailProduction findById(int id) {
		...
		Query query= session.createQuery("select c from DetailProduction c where c.id=:id");
	...
	public List<DetailProduction> ListHeursDetailProductionParDepotparJourGroupByEmployer(Date dateDebut,  int depot,String statut) {
		...
		Query query=null;
				 ...
				 query= session.createQuery("select c from DetailProduction c where c.production.date=:dateDebut and c.employe.depot.id=:depot and c.production.statut=:statut group by c.employe");
					query.setParameter("dateDebut", dateDebut);					 
					query.setParameter("depot", depot);
					query.setParameter("statut", statut);
