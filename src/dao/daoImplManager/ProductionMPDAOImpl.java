...
public class ProductionMPDAOImpl implements ProductionMPDAO {
	...
	@Override
	public List<ProductionMP> listeProductionMPTerminerbyDepotEntreDeuxDate(Date dateDebut,Date dateFin,String statut,String depot) {
		
		Query query= session.createQuery("select p from ProductionMP p where dateProduction >=:dateDebut and dateProduction <=:dateFin and statut =:statut and codeDepot=:depot");
	...
	@Override
	public List<ProductionMP> listeProductionMPGroupByArticle() {
		
		Query query= session.createQuery("select p from ProductionMP p where statut=:statut group by articlesMP.id order by  p.articlesMP.liblle");
