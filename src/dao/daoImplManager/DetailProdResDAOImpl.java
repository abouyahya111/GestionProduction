...
public class DetailProdResDAOImpl implements DetailProdResDAO {
	...
	public List<DetailProdRes> ListHeursDetailResponsableProdParDepotParJourGroupByEmployee(Date dateDebut , int depot ) {
		...
		Query query= null;
				 ...
				 query= session.createQuery("select c from DetailProdRes c where c.dateProduction  =:dateDebut   and c.employe.depot.id=:depot  group by c.employe");
					query.setParameter("dateDebut", dateDebut);
