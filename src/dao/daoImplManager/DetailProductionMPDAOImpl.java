...
public class DetailProductionMPDAOImpl implements DetailProductionMPDAO {
	...
	public List<DetailProductionMP> ListHeursDetailProductionMPParDepotParJourGroupByEmployee(Date dateDebut, int depot  ,String statut) {
		...
		Query query=null;
				 ...
				 query=session.createQuery("select c from DetailProductionMP c where c.productionMP.dateProduction  =:dateDebut  and c.employe.depot.id=:depot and c.productionMP.statut=:statut group by c.employe ");
					query.setParameter("dateDebut", dateDebut);
					query.setParameter("depot", depot);
					query.setParameter("statut", statut);
