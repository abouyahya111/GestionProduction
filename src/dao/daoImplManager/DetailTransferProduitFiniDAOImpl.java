...
public class DetailTransferProduitFiniDAOImpl implements DetailTransferProduitFiniDAO {
	...
	public List<DetailTransferProduitFini> lesArticleSortieBydateByArticleByTypeBayMagasin(Articles article ,String sortie,String sortieEnAttente,Magasin magasin,Date dateDebut) {
		...
		// TODO Auto-generated method stub
				Query query= session.createQuery("select d from DetailTransferProduitFini d where article.id =:article and (typeTransfer=:sortie or typeTransfer=:sortieEnAttente)  and magasinSouce.id=:magasin and transferStockPF.dateTransfer>=:dateDebut");
				query.setParameter("sortie", sortie);
				query.setParameter("sortieEnAttente", sortieEnAttente);
				query.setParameter("article", article.getId());
				query.setParameter("magasin", magasin.getId());
				query.setParameter("dateDebut", dateDebut);
...
/*	@Override
	...
	public List<DetailTransferProduitFini> ListTransferStockPFEntreDeuxDatesBYPFStatutX(Date dateDebut , Date dateFin , Articles article,String statut,Magasin magasin, Client client) {
		...
		{
			 if(statut.equals(Constantes.ETAT_TRANSFER_STOCK_SORTIE)  || statut.equals(Constantes.STATUT_TRANSFER_PRODUIT_FINI_SORTIE_ENATTENTE))
			 {
		 ...
		 if(dateDebut!=null && dateFin!=null )
		{
			 if(statut.equals(Constantes.ETAT_TRANSFER_STOCK_SORTIE))
			 {
				 ...
				 if(client!=null)
				 {
					 ...
					 if(article==null)
					 {
						 query= session.createQuery("select d from DetailTransferProduitFini d where transferStockPF.dateTransfer between :dateDebut and :dateFin and  transferStockPF.statut=:statut and magasinDestination.id=:magasin and client.id=:client");

						 query.setParameter("client", client.getId());
					 }else
					 {
						 query= session.createQuery("select d from DetailTransferProduitFini d where transferStockPF.dateTransfer between :dateDebut and :dateFin and  transferStockPF.statut=:statut and magasinDestination.id=:magasin and client.id=:client and article.id=:article ");
			...
			if(statut.equals(Constantes.ETAT_TRANSFER_STOCK_SORTIE))
			 {
				 ...
				 if(client!=null)
				 {
					 if(article==null)
					 {
						 query= session.createQuery("select d from DetailTransferProduitFini d where  transferStockPF.dateTransfer =:dateDebut and  transferStockPF.statut=:statut and magasinDestination.id=:magasin and client.id=:client");
						 query.setParameter("client", client.getId());
						 
					 }else
					 {
						 query= session.createQuery("select d from DetailTransferProduitFini d where  transferStockPF.dateTransfer =:dateDebut and  transferStockPF.statut=:statut and magasinDestination.id=:magasin and client.id=:client and article.id=:article");
						 query.setParameter("client", client.getId());
						 query.setParameter("article", article.getId());
	...
	if(dateDebut!=null && dateFin!=null )
		...
		if(statut.equals(Constantes.ETAT_TRANSFER_STOCK_SORTIE))
		{
			if(client!=null)
			{
				 if(article==null)
				 {
					 query= session.createQuery("select d from DetailTransferProduitFini d where transferStockPF.statut=:statut and  transferStockPF.dateTransfer between :dateDebut and :dateFin and magasinDestination.id=:magasin and client.id=:client group by article");
					 query.setParameter("client", client.getId());
					 
				 }else
				 {
					 query= session.createQuery("select d from DetailTransferProduitFini d where transferStockPF.statut=:statut and  transferStockPF.dateTransfer between :dateDebut and :dateFin and magasinDestination.id=:magasin and client.id=:client and article.id=:article group by article");
					 query.setParameter("client", client.getId());
					 query.setParameter("article", article.getId());
				 }
			...
			if(statut.equals(Constantes.ETAT_TRANSFER_STOCK_SORTIE))
			{
				if(client!=null)
				{
					 if(article==null)
					 {
						 ...
						 query= session.createQuery("select d from DetailTransferProduitFini d where transferStockPF.dateTransfer =:dateDebut and  transferStockPF.statut=:statut and magasinDestination.id=:magasin and client.id=:client group by article");
						 query.setParameter("client", client.getId()); 
					 }else
					 {
						 query= session.createQuery("select d from DetailTransferProduitFini d where transferStockPF.dateTransfer =:dateDebut and  transferStockPF.statut=:statut and magasinDestination.id=:magasin and client.id=:client and article.id=:article group by article");
						 query.setParameter("client", client.getId());
						 query.setParameter("article", article.getId());
					 }
...
/*	@Override
	...
	public List<DetailTransferProduitFini> findDetailTransferStockPFByRequette(String req) {
		...
		// TODO Auto-generated method stub
		Query query= session.createQuery("select d from DetailTransferProduitFini d "+req);
	...
	public List<Object[]> SommeTotalMontantTransfertMPPF(String typetransfer) {
		...
		Query query=null;
			...
			query= session.createQuery("select d.id, sum(d.quantite * d.prixUnitaire) FROM DetailTransferProduitFini d where d.typeTransfer =:typetransfer");
		
			query.setParameter("typetransfer", typetransfer);
	...
	public List<Object[]> SommeTotalMontantTransfertMPPFParArticle(String typetransfer) {
		...
		Query query=null;
			...
			query= session.createQuery("select d.article, sum(d.quantite), sum(d.quantite * d.prixUnitaire) FROM DetailTransferProduitFini d where d.typeTransfer =:typetransfer group by d.article");
